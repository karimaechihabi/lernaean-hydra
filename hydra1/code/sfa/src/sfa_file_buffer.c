...
enum response sfa_file_buffer_init(struct sfa_node *node)
{
  ...
  node->file_buffer = malloc(sizeof(struct sfa_file_buffer));

  if(node->file_buffer == NULL) {
    fprintf(stderr,"Error in sfa_file_buffer.c: Could not allocate memory for file buffer.\n");    
    return FAILURE;	
  }
...
struct sfa_record * get_all_time_series_in_node(struct sfa_trie * trie, struct sfa_node * node) 
{
  ...
  struct sfa_record * ret = NULL;
  
  ret = calloc(max_leaf_size, sizeof(struct sfa_record) );
     
  if (node->file_buffer->disk_count > 0)
  {      
      if(node->filename == NULL)
      {   
        fprintf(stderr, "Error in sfa_file_buffer.c: This node has data on disk but"
                        "could not get its filename.\n");
      }

     int full_size = strlen(trie->settings->root_directory) + strlen(node->filename)+1; 
     
     const char *full_filename = malloc(sizeof(char) * full_size);
     full_filename = strcpy(full_filename, trie->settings->root_directory);
     ...
     if(ts_file == NULL)
     {   
        fprintf(stderr, "Error in sfa_file_buffer.c: Could not open"
     ...
     if(fclose(ts_file))
     {   
        fprintf(stderr, "Error in sfa_file_buffer.c: Could not close"
		"the filename %s. Reason= %s.\n", ts_file_name, strerror(errno));
...
struct sfa_record * get_all_records_in_node(struct sfa_trie * trie, struct sfa_node * node) 
{
  ...
  {      
     ...
     const char * sfa_file_name = malloc(sizeof(char) * (full_size + 5) );;
     ...
     sfa_file_name = strcpy(sfa_file_name,full_filename);     
     sfa_file_name = strcat(sfa_file_name, ".sfa\0");
     ...
     FILE *sfa_file = fopen(sfa_file_name, "r");       
     COUNT_PARTIAL_INPUT_TIME_END
     ...
     {       
       ...
       ret[i].sfa_word = calloc(transforms_size, sizeof(unsigned char));
       
       //ret[i] = (ts_type *) trie->buffer_manager->current_record;
       //trie->buffer_manager->current_record += sizeof(ts_type) * ts_length;
       //trie->buffer_manager->current_record_trie++;
       ...
       fread(ret[i].sfa_word,
	     sizeof(unsigned char),transforms_size, sfa_file);
       COUNT_PARTIAL_INPUT_TIME_END       
     }
     ...
     for (int i = 0 ; i < idx; ++i)
     {
         ...
         ret[k].sfa_word = calloc(transforms_size, sizeof(unsigned char));
	 ...
	 for (int g = 0; g < transforms_size;++g)
	 {
	   ...
	   ret[k].sfa_word[g] = node->file_buffer->buffered_list[i].sfa_word[g];
	 }  	 
     }
  ...
  else
  {
	 ...
	 for (int g = 0; g < transforms_size;++g)
	 {
	   ...
	   ret[i].sfa_word[g] = node->file_buffer->buffered_list[i].sfa_word[g];
	 }  
     }
...
/*
 if full_flush, flush the raw, fft and sfa data. This is necessary 
 when the trie is still being built.
 Once the trie is final, a full flush is not needed as only the raw
 ts files are kept as the trie already encodes the necessary fft and sfa data.
...
enum response flush_buffer_to_disk(struct sfa_trie *trie, struct sfa_node *node, boolean full_flush)
{
  ...
  if (node->file_buffer->buffered_list_size > 0 )
  {
    ...
    if(node->filename == NULL)
    {   
        fprintf(stderr, "Error in sfa_file_buffer.c: Cannot flush the node to disk. "
     ...
     if(ts_file == NULL)
     {   
        fprintf(stderr, "Error in sfa_file_buffer.c: Flushing node to disk.."
		"Could not open the filename %s. Reason= %s\n", ts_file_name, strerror(errno));
        return SUCCESS;
     }

     const char *fft_file_name;
     const char *sfa_file_name;
     FILE *fft_file;
     FILE *sfa_file;
     ...
     if (full_flush)
     {
       ...
       sfa_file_name = malloc(sizeof(char) * (full_size + 5) );;     
       fft_file_name = strcpy(fft_file_name,full_filename);     
       fft_file_name = strcat(fft_file_name, ".fft\0");
       sfa_file_name = strcpy(sfa_file_name,full_filename);     
       sfa_file_name = strcat(sfa_file_name, ".sfa\0");
       ...
       sfa_file = fopen(sfa_file_name, "a");
       COUNT_PARTIAL_OUTPUT_TIME_END
    ...
    for (int idx = 0; idx < num_ts;++idx )
    {
      ...
      if(!fwrite(node->file_buffer->buffered_list[idx].timeseries, sizeof(ts_type), trie->settings->timeseries_size, ts_file))
      {   
        fprintf(stderr, "Error in sfa_file_buffer.c: Could not "
      ...
      if (full_flush)
	{
	  COUNT_PARTIAL_SEQ_OUTPUT
	    COUNT_PARTIAL_SEQ_OUTPUT
	    if(!fwrite(node->file_buffer->buffered_list[idx].dft_transform, sizeof(ts_type), trie->settings->fft_size, fft_file))
	      {   
		fprintf(stderr, "Error in sfa_file_buffer.c: Could not "
			"write the timeseries to file %s.\n", fft_file_name);
		return FAILURE;
	      }
	  if(!fwrite(node->file_buffer->buffered_list[idx].sfa_word, sizeof(unsigned char), trie->settings->fft_size, sfa_file))
	    {   
	      fprintf(stderr, "Error in sfa_file_buffer.c: Could not "
		      "write the timeseries to file %s.\n", sfa_file_name);
	      return FAILURE;
    ...
    if (!clear_file_buffer(trie, node))
    {
        fprintf(stderr, "Error in sfa_file_buffer.c: Flushing node to disk.. "
...
enum response clear_file_buffer(struct sfa_trie *trie, struct sfa_node * node)
{
  ...
  if ((node->file_buffer) == NULL )
  {
      fprintf(stderr, "Error in sfa_file_buffer.c: Cannot clear a NULL buffer.\n");
      return FAILURE;          
  }
  else
  {
...
enum response delete_file_buffer(struct sfa_trie * trie,struct sfa_node * node)
{
  ...
  if (node->file_buffer->in_disk) //delete file if in disk
  {
    ...
    // fprintf(stderr, "In sfa_file_buffer.c: Deleting node %s from disk.\n.", full_filename);
     
    if(!remove(ts_file_name) && !remove(fft_file_name) && !remove(sfa_file_name)) //file deleted successfully
    {
      node->file_buffer->disk_count = 0;
      node->file_buffer->in_disk = false;
    }
    ...
    {
      fprintf(stderr, "Error in sfa_file_buffer.c: Error deleting filename %s.\n", ts_file_name);
      return FAILURE;          
    }
    free(ts_file_name);  
    free(fft_file_name);
    ...
    struct sfa_file_map * res = node->file_buffer->position_in_map;
    
    if (res != NULL)
    {
       if (res->prev == NULL) //first element in file map 
       { 
         trie->buffer_manager->file_map = res->next;
	 ...
	 {
	   trie->buffer_manager->file_map_tail = NULL;
	 }
       }
       else if (res->next == NULL) //deleting the last element in the map
       {
         res->prev->next = NULL;
	 trie->buffer_manager->file_map_tail = res->prev;   
       }
       else
       {
       ...
       if (!clear_file_buffer(trie, node))
       {
         fprintf(stderr, "Error in sfa_file_buffer.c: Deleting node.. "
                          "Could not clear the buffer for %s.\n", node->filename);
         return FAILURE;      
       }
