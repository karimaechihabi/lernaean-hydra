...
int main ()
{
    ...
    size_t nb = 1 * 1000 * 1000;

    // make the index object and train it
    faiss::IndexFlatL2 coarse_quantizer (d);

    // a reasonable number of centroids to index nb vectors
    // ncentroids = x, x can go from 4 to 16
    int ncentroids = int (4 * sqrt (nb));

    // make a set of nt training vectors in the unit cube
    // (could be the database)
    //from 30 * x to 256 * x 
    size_t nt = 40 * ncentroids;
    ...
    faiss::IndexIVFPQ index (&coarse_quantizer, d,
                             ncentroids, nbytes_code, nbits_subcode);
    ...
    { // I/O demo
        const char *outfilename = "index/index_trained.faissindex";
    ...
    { // I/O demo
        const char *outfilename = "index/index_populated.faissindex";
        printf ("[%.3f s] storing the pre-trained index to %s\n",
                elapsed() - t0, outfilename);
