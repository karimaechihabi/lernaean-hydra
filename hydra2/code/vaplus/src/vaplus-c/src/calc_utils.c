...
/*
  Using the variance computational formula.
...
ts_type calc_variance (ts_type * series, int start, int end)
{
  ...
  ts_type sum_x_squares=0, sum_x=0, variance = 0; //sum of x's and sum of x squares
  int i, count_x;

  if (start >= end)
  {
    printf ("error in variance start >= end\n");
  }
  ...
  else
  {
    ...
    variance = sum_x_squares/count_x;
  }
