...
int main (int argc, char** argv){
  ...
  sw_log << "done " << (end.tv_sec - start.tv_sec) / 60 << " minutes" << endl;
  ...
  sw_log <<"allocating memory for sums...";
  sw_log.flush();
  clock_gettime(CLOCK_MONOTONIC, &start);
  ...
  sw_log << "allocating memory for signs matrix...";
  sw_log.flush();

  clock_gettime(CLOCK_MONOTONIC, &start);
  signs =  alloc_1d_or_2d_matrix(signs, signs_dim1, signs_dim2);
  clock_gettime(CLOCK_MONOTONIC, &end);
  sw_log << "done " << (end.tv_sec - start.tv_sec)/60 << " minutes" << endl;

  sw_log << "reading signs...";
  sw_log.flush();

  clock_gettime(CLOCK_MONOTONIC, &start);
  read_signs(sk_sr, signs, signs_dim1, signs_dim2, program_size);
  clock_gettime(CLOCK_MONOTONIC, &end);
  sw_log << "done " << (end.tv_sec - start.tv_sec)/60 << " minutes" << endl;
  sw_log.flush();
  ...
  delete [] sums;
  free_1d_or_2d_matrix(signs, signs_dim1, signs_dim2);
